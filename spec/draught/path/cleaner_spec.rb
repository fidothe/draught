  ...
  RSpec.describe Path::Cleaner do
    ...
    describe "cleaning a path by removing extra identical points" do
      specify "returns a new cleaned path" do
        input = world.path.simple(points: [p(0,0), p(1,0), p(1,0), p(4,0)])
        expected = world.path.simple(points: [p(0,0), p(1,0), p(4,0)])

        expect(described_class.dedupe(world, input)).to eq(expected)
      end

      context "handling Metadata" do
        let(:input) { world.path.simple(points: [p(0,0), p(4,0), p(1,0)], metadata: metadata) }
    ...
    describe "simplifying a path" do
      let(:expected) { world.path.simple(points: [p(0,0), p(4,0), p(4,4)]) }

      it "removes unnecessary intermediate points on the horizontal" do
        input = world.path.simple(points: [p(0,0), p(1,0), p(1,0), p(2,0), p(3,0), p(4,0), p(4,4)])
      ...
      it "removes unnecessary intermediate points on the vertical" do
        input = world.path.simple(points: [p(0,0), p(4,0), p(4,1), p(4,2), p(4,4)])

        expect(described_class.simplify(world, input)).to eq(expected)
      end

      it "goes around corners" do
        input = world.path.simple(points: [p(0,0), p(1,0), p(3,0), p(4,0), p(4,2), p(4,4)])

        expect(described_class.simplify(world, input)).to eq(expected)
      end

      it "does not remove points where it's not obvious which point to remove" do
        input = world.path.simple(points: [p(0,0), p(4,0), p(1,0)])
        expected = world.path.simple(points: [p(0,0), p(4,0), p(1,0)])

        expect(described_class.simplify(world, input)).to eq(expected)
      end

      it "copes with paths proceeding right-to-left" do
        input = world.path.simple(points: [p(4,0), p(2,0), p(1,0)])
        expected = world.path.simple(points: [p(4,0), p(1,0)])

        expect(described_class.simplify(world, input)).to eq(expected)
      end

      it "copes with paths proceeding top-to-bottom" do
        input = world.path.simple(points: [p(0,4), p(0,2), p(0,1)])
        expected = world.path.simple(points: [p(0,4), p(0,1)])
