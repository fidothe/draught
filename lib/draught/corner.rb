  ...
  class Corner
    ...
    class Rounded
      ...
      def incoming_line_segment
        LineSegment.from_path(incoming[-2,2])
      end
      ...
      def outgoing_line_segment
        LineSegment.from_path(outgoing[0..1])
      end
      ...
      def join_angles
        @join_angles ||= JoinAngles.new(incoming_line_segment, outgoing_line_segment)
      end
    ...
    class JoinAngles
      attr_reader :incoming_line_segment, :outgoing_line_segment

      def initialize(incoming_line_segment, outgoing_line_segment)
      ...
      def corner_top_line_segment
        @corner_top_line_segment ||= begin
          corner_top_line_segment = LineSegment.build({
      ...
      def aligned_zeroed_line_segments
        @aligned_zeroed_line_segments ||= begin
          joined_line_segments = PathBuilder.connect(incoming_line_segment, outgoing_line_segment)
          zeroed_joined_line_segments = joined_line_segments.translate(Vector.translation_to_zero(joined_line_segments.first))
