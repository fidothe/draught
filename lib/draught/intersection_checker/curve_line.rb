...
require_relative '../approximate_range'
  ...
  module IntersectionChecker
    class CurveLine
      ...
      def self.check(segment_1, segment_2)
        ...
        new(curve_segment, line_segment).check
      end

      attr_reader :curve_segment, :line_segment

      def initialize(curve_segment, line_segment)
        @curve_segment, @line_segment = curve_segment, line_segment
      end
      ...
      def x_intersection_range
        ApproximateRange.new(line_segment.lower_left.x..line_segment.lower_right.x)
      end

      def y_intersection_range
        ApproximateRange.new(line_segment.lower_left.y..line_segment.upper_left.y)
      end
      ...
      def align_transform
        @align_transform ||= Transformations::Composer.compose(
          Vector.translation_between(line_segment.start_point, Point::ZERO),
